-- Tabelas para integração com Mercado Pago
...
-- Tabela para armazenar preferências de pagamento
...
-- Tabela para logs de webhook (opcional, para auditoria)
...
-- Tabela para configurações do Mercado Pago (opcional)
...
INSERT IGNORE INTO `mercadopago_settings` (`setting_key`, `setting_value`, `description`) VALUES
...
ALTER TABLE `mercadopago_transactions` ADD INDEX `idx_payment_lookup` (`payment_id`, `status`);
ALTER TABLE `mercadopago_preferences` ADD INDEX `idx_user_active` (`user_id`, `status`);
ALTER TABLE `mercadopago_webhook_logs` ADD INDEX `idx_resource_lookup` (`type`, `resource_id`);

-- Comentários nas tabelas
ALTER TABLE `mercadopago_transactions` COMMENT = 'Armazena todas as transações do Mercado Pago';
ALTER TABLE `mercadopago_preferences` COMMENT = 'Armazena preferências de pagamento criadas';
ALTER TABLE `mercadopago_webhook_logs` COMMENT = 'Logs de webhooks recebidos do Mercado Pago';
ALTER TABLE `mercadopago_settings` COMMENT = 'Configurações do sistema Mercado Pago';