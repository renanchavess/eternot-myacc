    ...
    if (!$logged) {
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        <div style="float: right;">
                                            <form action="?account/manage" method="post" style="padding:0px;margin:0px;">
                                                <div class="BigButton" style="background-image:url({$template_path}/images/global/buttons/sbutton.gif)">
                                                    <div onmouseover="MouseOverBigButton(this);" onmouseout="MouseOutBigButton(this);"><div class="BigButtonOver" style="background-image: url({$template_path}/images/global/buttons/sbutton_over.gif); visibility: hidden;"></div>
                                        ...
                                        <p>
                                            <b>Log in</b> to see all <b>your bids</b> for character auctions.
                                        </p>
                                    </td>
                                </tr>
                            </tbody>
        ...
        echo <<<HTML
            ...
            HTML;
                                            ...
                                            $getBidsbyAccount = [];
                                            if ($logged) {
                                                $getBidsbyAccount = $db->query("SELECT `id`, `account_id`, `auction_id`, `bid`, `date` FROM `myaac_charbazaar_bid` WHERE `account_id` = {$account_logged->getId()} ORDER BY `date` DESC");
                                                $i_bg = 0;
                                                foreach ($getBidsbyAccount as $BidbyAccount) {
                                                    ...
                                                    <tr bgcolor="<?= getStyle($i_bg); ?>">
                                                        ...
                                                        <td style="color: <?= $bg_DateEnd ?>;">
                                                                <?php if ($getAuctionbyBid['status'] == 0) {
                                                                    echo "Waiting";
                                                                } else {
                                                                    echo "<img src='{$status_icon}'>";
                                                                }?>
                                                        </td>
                                                        <td style="color: <?= $bg_DateEnd ?>;"><?= number_format($BidbyAccount['bid'], 0, ',', ','); ?>
                                                            <img
